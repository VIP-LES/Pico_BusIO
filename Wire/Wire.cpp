...
bool TwoWire::setSDA(pin_size_t pin) {
  ...
  if (_running) {
    panic("FATAL: Attempting to set Wire%s.SDA while running",
          i2c_hw_index(_i2c) ? "1" : "");
  }
  return false;
}
...
bool TwoWire::setSCL(pin_size_t pin) {
  ...
  if (_running) {
    panic("FATAL: Attempting to set Wire%s.SCL while running",
          i2c_hw_index(_i2c) ? "1" : "");
  }
  return false;
}
...
void TwoWire::begin(uint8_t addr) {
  ...
  // Our callback IRQ
  _i2c->hw->intr_mask =
      (1 << 12) | (1 << 10) | (1 << 9) | (1 << 6) | (1 << 5) | (1 << 2);

  int irqNo = I2C0_IRQ + i2c_hw_index(_i2c);
  irq_set_exclusive_handler(irqNo,
                            i2c_hw_index(_i2c) == 0 ? _handler0 : _handler1);
...
// See:
// https://github.com/earlephilhower/arduino-pico/issues/979#issuecomment-1328237128
...
size_t TwoWire::requestFrom(uint8_t address, size_t quantity, bool stopBit) {
  ...
  _buffLen = i2c_read_blocking_until(_i2c, address, _buff, quantity, !stopBit,
                                     make_timeout_time_ms(_timeout));
...
size_t TwoWire::write(uint8_t ucData) {
  ...
  if (_slave) {
    ...
    while ((i2c_get_write_available(_i2c) == 0) &&
           !time_reached(end)) { /* noop wait */
    }
...
// DMA/asynchronous transfers.  Do not combime with synchronous runs or bad
// stuff will happen All buffers must be valid for entire DMA and not touched
// until `finishedAsync()` returns true.
bool TwoWire::writeReadAsync(uint8_t address, const void *wbuffer,
                             size_t wbytes, const void *rbuffer, size_t rbytes,
                             bool sendStop) {
  ...
  // Create or enlarge dma command buffer, we need one entry for every i2c byte
  // we want to write/read
  ...
  channel_config_set_transfer_data_size(
      &c, DMA_SIZE_16);                        // 16b transfers into I2C FIFO
  channel_config_set_read_increment(&c, true); // Reading incrementing addresses
  channel_config_set_write_increment(&c,
                                     false); // Writing to the same FIFO address
  channel_config_set_dreq(
      &c, i2c_get_dreq(_i2c, true)); // Wait for the TX FIFO specified
  channel_config_set_chain_to(&c, _dmaChannelSend); // No chaining
  channel_config_set_irq_quiet(
      &c, false); // Enable interrupt (can be disabled later with
                  // dma_channel_set_irq0_enabled() as needed)
  dma_channel_configure(_dmaChannelSend, &c, &_i2c->hw->data_cmd,
                        _dmaSendBuffer, wbytes + rbytes, false);
  ...
  channel_config_set_transfer_data_size(
      &c, DMA_SIZE_8);                          // 8b transfers from I2C FIFO
  ...
  channel_config_set_dreq(
      &c, i2c_get_dreq(_i2c, false)); // Wait for the RX FIFO specified
  channel_config_set_chain_to(&c, _dmaChannelReceive); // No chaining
  channel_config_set_irq_quiet(
      &c, false); // Enable interrupt (can be disabled later with
                  // dma_channel_set_irq0_enabled() as needed)
  dma_channel_configure(_dmaChannelReceive, &c, (void *)rbuffer,
                        &_i2c->hw->data_cmd, rbytes, false);

  // Enable dma completed interrupt
  dma_channel_set_irq0_enabled(
      _dmaChannelSend, (rbytes == 0)); // write only, enable irq on Send channel
  dma_channel_set_irq0_enabled(
      _dmaChannelReceive,
      (rbytes > 0)); // when reading, enable irq on Receive channel

  // Setup i2c hardware
  _i2c->hw->enable = 0;
  _i2c->hw->tar = address;
  _i2c->hw->dma_cr =
      1 << 1 | (rbytes > 0 ? 1 : 0); // TDMAE + RDMAE when rbytes>0
  _i2c->hw->enable = 1;
...
bool TwoWire::writeAsync(uint8_t address, const void *buffer, size_t bytes,
                         bool sendStop) {
  return writeReadAsync(address, buffer, bytes, nullptr, 0, sendStop);
}

bool TwoWire::readAsync(uint8_t address, void *buffer, size_t bytes,
                        bool sendStop) {
...
// Low level dma completed handlers, calls TwoWire::_dma_irq_handler() to do the
// work
...
void TwoWire::beginAsync() {
  ...
  if (i2c_hw_index(_i2c) == 0) {
    ...
    irq_add_shared_handler(DMA_IRQ_0, _dma_i2c0_irq_handler,
                           PICO_SHARED_IRQ_HANDLER_DEFAULT_ORDER_PRIORITY);
  } else {
    _dma_i2c1_irq_mask = (1u << _dmaChannelReceive) | (1u << _dmaChannelSend);
    _dma_i2c1_wire_instance = this;
    irq_add_shared_handler(DMA_IRQ_0, _dma_i2c1_irq_handler,
                           PICO_SHARED_IRQ_HANDLER_DEFAULT_ORDER_PRIORITY);
  }
...
size_t TwoWire::setBufferSize(size_t bSize) {
  ...
  // only free the buffer, if it already exists and the new size differs from
  // the current one
  ...
  _buffSize =
      max(WIRE_BUFFER_SIZE_MIN, int(bSize)); // enforce minimum buffer size
  return _buffSize;
}
