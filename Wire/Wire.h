...
#define WIRE_BUFFER_SIZE                                                       \
  256 // default size, if none is set using Wire::setBuffersize(size_t)
#endif
...
public:
  ...
  bool writeAsync(uint8_t address, const void *buffer, size_t bytes,
                  bool sendStop = true);
  bool readAsync(uint8_t address, void *buffer, size_t bytes,
                 bool sendStop = true);
  bool busIdle();
  bool finishedAsync(); // Call to check if the async operations is completed
                        // and the buffer can be reused/read
  void abortAsync();    // Cancel an outstanding async I2C operation
  void
  onFinishedAsync(void (*function)(void)); // Set callback for async operation
  void _dma_irq_handler(); // "private" method, made public to call this method
                           // from low level dma irq handler

  void setTimeout(uint32_t timeout = 25,
                  bool reset_with_timeout =
                      false); // sets the maximum number of milliseconds to wait
  bool getTimeoutFlag(void);
  void clearTimeoutFlag(void);

  size_t
  setBufferSize(size_t bSize); // set buffer size (call prior to 'begin()')
...
private:
  ...
  uint16_t *_dmaSendBuffer =
      nullptr; // dma command send buffer (dynamically allocated)
  ...
  void (*_dmaOnFinished)(void) =
      nullptr;       // user handler to call on dma completion
  void beginAsync(); // setup dma channels and irq, called on first use of an
                     // async read/write function
