...
public:
  /**
      @brief Create a PIO-based SPI instance, pins can be changed before begin()
     call
  ...
  */
  ...
  /**
      @brief Send a 16-bit quantity over SPI and return read-back 16-bit value
  ...
  */
  ...
  /**
      @brief Sends buffer in 8 bit chunks under a single CS. Overwrites buffer
     with read data
  ...
  */
  ...
  /**
      @brief Sends one buffer and receives into another under a single CS. Can
     set rx or txbuf to nullptr

      @param [in] txbuf Buffer to transmit or nullptr to send 0s
      @param [out] rxbuf Buffer to read back into or nullptr to ignore returned
     data
  ...
  */
  ...
  // until `finished()` returns true.
  /**
      @brief Perform a transfer() using DMA in the background. Returns
     immediately, need to check for completion

      @details
      Do not combine asynchronous and synchronous transfers.  All buffers must
     be valid until the transfer reports that it is completed (``finished``
     returns true).

      @param [in] send Buffer to transmit, must remain valid through entire
     operation
      @param [out] recv Buffer to receive, must remain valid through entire
     operation
      @param [in] bytes Number of bytes to transfer under single CS
  */
  bool transferAsync(const void *send, void *recv, size_t bytes);
  /**
      @brief Call to check if the async operations is completed and the buffer
     can be reused/read

      @returns True if the asynchronous SPI operation has completed and ``recv``
     buffer is valid
  */
  bool finishedAsync();

  /**
      @brief Aborts an ongoing asynchronous SPI operation, if one is still
     operating

      @details
      Not normally needed, but in the case where a large, long SPI operation
     needs to be aborted this call allows an application to safely stop the SPI
     and dispose of the ``recv`` and
      ``send`` buffers
  */
  void abortAsync();

  /**
      @brief Begin an SPI transaction, sets SPI speed and masks necessary
     interrupts

      @param [in] SPISettings SPI configuration parameters, including the clock
     speed
  */
  ...
  */
  ...
  /**
      ...
      @param [in] hwCS Pass in true to enable HW-controlled CS.  Otherwise
     application needs to assert/deassert CS.
  */
  ...
  */
  ...
  /**
      @brief Ensure specific GPIO interrupt is disabled during and SPI
     transaction to protect against re-entrancy.  Multiple GPIOs supported by
     multiple calls.
